package services.matchManagement;
...
public class EquipeService {
    ...
    /**
     * Joueur sans équipe. Si la colonne equipe_id est NOT NULL, on ne peut pas mettre NULL;
     * les vraies équipes ont un id {@code > 0}. Si une FK interdit 0, créer une ligne sentinelle ou retirer la FK.
     */
    private static final int PLAYER_EQUIPE_NONE = 0;
    ...
    private void updatePlayersForEquipe(int equipeId, List<Player> players) throws SQLException {
        // Retirer d'abord les joueurs de cette équipe (NOT NULL: utiliser 0, pas NULL)
        String resetSql = "UPDATE player SET equipe_id = ? WHERE equipe_id = ?";
        PreparedStatement psReset = cnx.prepareStatement(resetSql);
        psReset.setInt(1, PLAYER_EQUIPE_NONE);
        psReset.setInt(2, equipeId);
    ...
    private void ensurePlayerExists(int playerId) throws SQLException {
        ...
        ResultSet rs = ps.executeQuery();
        if (!rs.next()) {
            String insertSql = "INSERT INTO player (id, statut, pays, niveau, equipe_id) VALUES (?, ?, ?, ?, ?)";
            PreparedStatement psInsert = cnx.prepareStatement(insertSql);
            psInsert.setInt(1, playerId);
            psInsert.setBoolean(2, true);
            psInsert.setString(3, "Non spécifié");
            psInsert.setString(4, "Beginner");
            psInsert.setInt(5, PLAYER_EQUIPE_NONE);
            psInsert.executeUpdate();
        }
    }
    ...
    public Equipe getById(int id) {
        String sql = "SELECT e.*, u.email as coach_email, c.specialite as coach_specialite, c.pays as coach_pays " +
                     "FROM equipe e " +
                     "LEFT JOIN coach c ON e.coach_id = c.id " +
                     "LEFT JOIN user u ON c.id = u.id " +
                     "WHERE e.id = ?";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                int cid = rs.getInt("coach_id");
                if (!rs.wasNull() && cid > 0) {
                    Coach coach = new Coach();
                    coach.setId(cid);
                    if (rs.getString("coach_specialite") != null) {
                        coach.setSpecialite(rs.getString("coach_specialite"));
                    }
                    if (rs.getString("coach_pays") != null) {
                        coach.setPays(rs.getString("coach_pays"));
                    }
                    User u = new User();
                    u.setId(cid);
                    u.setEmail(rs.getString("coach_email"));
    ...
    public boolean deleteEquipe(int id) {
        ...
        try {
            ...
            // 2. Détacher les joueurs (NOT NULL: 0 = sans équipe)
            String unassignPlayersSql = "UPDATE player SET equipe_id = ? WHERE equipe_id = ?";
            PreparedStatement psUnassign = cnx.prepareStatement(unassignPlayersSql);
            psUnassign.setInt(1, PLAYER_EQUIPE_NONE);
            psUnassign.setInt(2, id);
