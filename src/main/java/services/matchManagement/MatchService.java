...
public class MatchService {
    ...
    public void add(Matchs m) {
        ...
        try {
            ...
            System.out.println("Match ajouté 🔥 (ID: " + m.getId() + ")");
    ...
    /**
     * Matchs dont la date de début est dans [fromInclusive, toInclusive], ordre chronologique.
     * Utilisé pour l’export calendrier (ex. 10 dernières années).
     */
    public List<Matchs> findMatchesBetween(LocalDateTime fromInclusive, LocalDateTime toInclusive) {
        List<Matchs> list = new ArrayList<>();
        String sql = """
                SELECT
                    m.*,
                ...
                JOIN equipe e1 ON m.equipe1_id = e1.id
                JOIN equipe e2 ON m.equipe2_id = e2.id
                WHERE m.date_match IS NOT NULL
                  AND m.date_match >= ?
                  AND m.date_match <= ?
    ...
    private Matchs mapRowToMatch(ResultSet rs) throws SQLException {
        Matchs m = new Matchs();
        m.setId(rs.getInt("id"));
        m.setNomMatch(rs.getString("nom_match"));
        m.setScoreEquipe1((Integer) rs.getObject("score_equipe1"));
        m.setScoreEquipe2((Integer) rs.getObject("score_equipe2"));
