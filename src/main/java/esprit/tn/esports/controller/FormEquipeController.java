...
public class FormEquipeController {
    ...
    private void initSelectors() {
        ...
        // Player Selector
        playersListView.setItems(FXCollections.observableArrayList(playerService.getAll()));
        ...
        playersListView.setCellFactory(lv -> new ListCell<Player>() {
            ...
            protected void updateItem(Player player, boolean empty) {
                ...
                if (empty || player == null || player.getUser() == null) {
                    ...
                    Label emailLabel = new Label(player.getUser().getEmail());
                    ...
                    badge.getStyleClass().add("badge-" + level);

                    hbox.getChildren().addAll(emailLabel, spacer, badge);
                    setGraphic(hbox);
                    setText(null);
                }
            }
        ...
        wirePlayersSelectionValidation();
    }

    private void wirePlayersSelectionValidation() {
        MultipleSelectionModel<Player> msel =
                (MultipleSelectionModel<Player>) (Object) playersListView.getSelectionModel();
        msel.getSelectedItems().addListener(this::onPlayersListInvalidated);
    }

    private void onPlayersListInvalidated(Observable o) {
        validatePlayers();
        validateForm();
    }
    ...
    public void setEquipe(Equipe e) {
        ...
        if (e != null) {
            ...
            // Select Players (même id que la liste; une fois trouvé, passer au membre suivant)
            ...
            if (teamMembers != null) {
                for (Player member : teamMembers) {
                    for (int i = 0; i < playersListView.getItems().size(); i++) {
                        if (playersListView.getItems().get(i).getId() == member.getId()) {
                            playersListView.getSelectionModel().select(i);
                            break;
                        }
                    }
                }
            ...
            try {
                if (imagePath != null) {
                    if (new File(imagePath).isFile()) {
                        ...
                        var stream = getClass().getResourceAsStream("/esprit/tn/esports/images/" + imagePath);
                        if (stream != null) {
                            preview.setImage(new Image(stream));
                        }
                    }
                }
            } catch (Exception ex) {
            ...
            // Après pré-sélection multi, rafraîchir l'état du bouton
    ...
    void save() {
        ...
        // Copie explicite (sélection UI évoluante) pour la persistance en base
        List<Player> selected = new ArrayList<>(playersListView.getSelectionModel().getSelectedItems());
        e.setPlayers(selected);
