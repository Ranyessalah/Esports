...
public class EditCoachProfileController implements Initializable {
    ...
    private boolean validateConfirm() {
        ...
        if (passwordField.getText().isEmpty() && v.isEmpty()) {
            clearError(confirmPasswordField, confirmError); return true;
        }
    ...
    private void onSave() {
        ...
        if (specialiteCombo.getValue() == null) {
            setError(null, specialiteError, "Veuillez sélectionner votre spécialité.");
            shake(specialiteCombo); valid = false;
        }
        ...
        try {
            ...
            currentCoach.setPays(paysCombo.getValue());
            currentCoach.setSpecialite(specialiteCombo.getValue());
            currentCoach.setDisponibilite(ouiRadio.isSelected());
