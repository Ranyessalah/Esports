...
public class MatchController {
    ...
    public void initialize() {
        ...
        MatchNotificationService notifService = MatchNotificationService.getInstance();
        
        notifService.setOnMatchWarning(m -> {
            try {
                System.out.println("[MatchController] ⏳ WARNING reçu: " + m.getNomMatch());
                pushNotification(m);
                MatchNotificationService.playAlertSound();
                ...
                if (window != null) {
                    MatchNotificationService.showToast(
                        window, 
                        "⏳ Match dans 15 min", 
...
// =====================
    private void openForm(Matchs match, boolean resultOnly) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/matchManagement/formMatch.fxml")
            );
    ...
    public void goStats(ActionEvent event) {
        ...
        try {
            ...
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/matchManagement/stats.fxml")
            );
