...
import entities.StatsRow;
import services.gestion_match.EquipeService;
import services.gestion_match.StatsService;
...
public class Statsclientcontroller {
    ...
    private void setCircularImage(ImageView iv, String path) {
        try {
            if (path != null && !path.isEmpty()) {
                ...
                iv.setImage(new javafx.scene.image.Image(getClass().getResourceAsStream("/images/default_team.png")));
            }
    ...
    public void goEquipes(javafx.event.ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/gestion_match/equipeIndex_client.fxml")
            );
            Parent root = loader.load();
    ...
    public void goMatchs(javafx.event.ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/gestion_match/matchIndex_client.fxml")
            );
            Parent root = loader.load();
            Stage stage = (Stage) ((javafx.scene.Node) event.getSource()).getScene().getWindow();
            stage.setScene(new Scene(root, 1200, 760));
